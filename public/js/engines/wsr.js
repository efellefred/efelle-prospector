...
function buildWSRReportHTML(d) {
  ...
  const cover = '<div style="background:#000;background-image:radial-gradient(ellipse at top right,rgba(5,150,105,0.2) 0%,#000 60%);padding:44px 64px 40px;">' +
    ...
    '</div>' +
    '<div style="font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:' + GRL + ';margin-bottom:16px;">Website Audit &amp; Recommendations</div>' +
    '<div style="font-size:48px;font-weight:800;color:#fff;line-height:1.1;margin-bottom:16px;">' + esc(d.client_name||'Website') + '</div>' +
    '<div style="font-size:15px;color:#AEAEB2;margin-bottom:0;">Website audit and digital health assessment &amp; recommendations for the current website.</div>' +
    ...
    '<div style="font-size:14px;color:#fff;line-height:1.8;">' + esc(d.prepared_for||d.client_name||'') + '<br>' + esc(d.website||'') + '</div></div>' +
    ...
    '<div style="font-size:13px;color:#6E6E73;">' + date + '</div></div></div></div>' +
    '</div>' +
  '</div>';
