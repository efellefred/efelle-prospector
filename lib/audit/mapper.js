...
function buildAICategory(r) {
  ...
  checks.push({
    passed: !!g.aiCrawlerAccess,
    severity: 'critical',
    issueTitle: 'AI Search Crawlers Blocked',
    issueDescription: g.blockedAiBots && g.blockedAiBots.length > 0
      ? `robots.txt blocks these AI crawlers: ${g.blockedAiBots.join(', ')}. Your content won't appear in AI-generated search answers.`
      : 'AI crawler access could not be determined.',
    recTitle: 'Allow AI Search Crawlers',
    recDescription: 'Update robots.txt to allow GPTBot, ClaudeBot, and PerplexityBot. Blocking these bots means your business is invisible in AI search results.',
  });
